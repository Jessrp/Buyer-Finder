...
(function () {
  ...
  function lastSeenLabel(ts) {
    ...
    return "⚠️ Last seen a long time ago";
  }
  ...
  async function load() {
    ...
    try {
      ...
      list.innerHTML = convos.map(c => {
        ...
        const typeDot   = `<span style="width:6px;height:6px;border-radius:50%;background:${typeColor};display:inline-block;margin-right:4px;vertical-align:middle;"></span>`;
          ...
          <div class="list-item" style="display:flex;align-items:center;gap:12px;cursor:pointer;padding:12px;"
            ...
            <div style="flex:1;min-width:0;">
              <div style="display:flex;justify-content:space-between;align-items:baseline;margin-bottom:2px;">
                <strong style="font-size:14px;">${otherName}${unreadBadge}</strong>
              ...
              ${lastSeen ? `<div style="font-size:10px;color:var(--muted);margin-top:3px;opacity:0.65;">${lastSeen}</div>` : ""}
            </div>
          </div>`;
      }).join("");
