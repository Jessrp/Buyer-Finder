...
(function () {
  ...
  async function refreshBadge() {
    const client = supa();
    const user = window.currentUser;
    const badge = document.getElementById("nav-alerts-badge") || document.getElementById("alerts-badge");
    if (!badge) return;

    if (!client || !user) { badge.textContent = ""; badge.classList.add("hidden"); return; }
