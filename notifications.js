...
(function () {
  ...
  // NEW: safely extract match id from either ref_id or meta JSON
  function getMatchIdFromAlert(alertRow) {
    ...
    // meta might already be an object, or a JSON string
    try {
  ...
  async function load() {
    ...
    list.innerHTML = alerts.map(a => {
      ...
      // Store ref/meta in dataset so click handler can route properly
      ...
      return `
        <div class="alert-item${unread}"
          ...
          <div style="display:flex;justify-content:space-between;gap:10px;align-items:flex-start;">
            <strong>${t}</strong>
            <span class="muted" style="opacity:.7;font-size:11px;">${fmtTime(a.created_at)}</span>
          </div>
          <div class="muted" style="opacity:.85;margin-top:6px;">${b}</div>
        </div>
      `;
    }).join("");
    ...
    list.querySelectorAll(".alert-item").forEach(el => {
      el.addEventListener("click", async () => {
        ...
        // reconstruct minimal row object for parser
        ...
        if (type === "message") {
          // Open conversation if possible (ref_id typically stores conversation id)
