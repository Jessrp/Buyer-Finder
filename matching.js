...
// This file is NOT a module. It attaches functions to window.Matching.
(function () {
  ...
  const MATCH_THRESHOLD = 30; // tweak as you like
  ...
  async function findMatchesForPost(post) {
    const supa = getSupa();
    if (!supa) {
      console.warn("Matching: supa not ready yet");
      return;
    }
  ...
  async function scanAndCreateMatchesForUser() {
    ...
    if (error || !mine) {
      console.error("scanAndCreateMatchesForUser: failed to load my posts", error);
      return;
    }
