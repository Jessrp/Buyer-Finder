...
(function () {
  ...
  // Inserts stored matches at/above this score (alerts typically come from DB triggers on matches inserts)
  ...
  // Allow a small set of 2-char tokens that are genuinely meaningful for items.
  // This is the “don’t delete TV/PC/VR” fix without allowing random 2-letter junk.
  const ALLOW2 = new Set([
    "tv","pc","vr","ps","xs","xl","sr","hd","4k"
  ]);
  ...
  function keepToken(t) {
    ...
    // Keep 2-char tokens only if:
    // - it contains a digit (e.g. "4k"), OR
    // - it's in the allowlist (tv/pc/vr/etc)
    if (t.length === 2) {
  ...
  function hardMatch(a, b) {
    ...
    // Keep your original rule: no self matching
  ...
  function scoreMatch(a, b) {
    ...
    // Title bonuses for obvious same-item pairs
    const ta = normText(a.title);
    const tb = normText(b.title);
    if (ta && tb) {
      if (ta === tb) score += 30;
      else if (ta.includes(tb) || tb.includes(ta)) score += 18;
      else {
    ...
    // Small description similarity booster
  ...
  async function insertMatchRow(buyPost, sellPost, score) {
    ...
    if (error) {
      ...
      // This is the most important debug output if RLS blocks inserts.
