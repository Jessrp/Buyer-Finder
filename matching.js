...
(function () {
  ...
  // Keep this low-ish so we can store "potential" matches, but not total garbage.
  ...
  function tokensFromPost(p) {
    ...
    return all
      ...
      .map(t => t.trim())
      .filter(Boolean)
      .filter(t => t.length >= 3)          // keep it strict to avoid junk
      .filter(t => !STOP.has(t))
  ...
  function hardMatch(a, b) {
    ...
    // If you want self-matching for testing, remove this line.
  ...
  function scoreMatch(a, b) {
    ...
    // --- MAIN CHANGE #1: overlap worth more ---
    // Old: overlap * 18 (needed 4 overlaps to hit 70)
    // New: overlap * 22 (3 overlaps becomes 66, then other bonuses can push it over)
    score += Math.min(80, overlap * 22);

    // --- MAIN CHANGE #2: title bonus for "obviously the same thing" ---
    const ta = normText(a.title);
    const tb = normText(b.title);
    if (ta && tb) {
      if (ta === tb) score += 30;                 // exact title match
      else if (ta.includes(tb) || tb.includes(ta)) score += 18; // one contains the other
      else {
    ...
    // small description similarity booster (safe)
