// messages.js — inbox + chat overlay, works with BuyerFinder posts.js (Conversations.getOrCreateConversation)
(function () {
  console.error("📨 MESSAGES.JS LOADED (fixed)");
  ...
  function addMessageToUI(msg) {
    ...
    div.style.background = mine ? "rgba(0,180,255,0.20)" : "rgba(255,255,255,0.08)";
    ...
    messageChannel = client
      ...
      .on(
        ...
        (payload) => {
          ...
          // Avoid double rendering if it's our own send (still safe, but we can keep it)
          if (!msg) return;
          addMessageToUI(msg);

          // In-app + browser notification for incoming
  ...
  // Expose for posts.js compatibility
  window.Messages = { loadInbox, openConversation };
  window.openConversation = openConversation; // posts.js calls this directly sometimes
})();
