...
(function () {
  console.error("📨 MESSAGES.JS LOADED (polished+instant-send)");

  function supa() { return window.supa; }

  let activeConversationId = null;
  let messageChannel = null;
  const seenIds = new Set(); // dedupe realtime vs immediate render
  ...
  async function sendActiveMessage() {
    ...
    // Immediately clear the input so it feels instant
    if (input) input.value = "";

    // Try to insert and return the inserted row so we can render immediately (no waiting for realtime)
    ...
    // Render immediately (dedupes if realtime also arrives)
