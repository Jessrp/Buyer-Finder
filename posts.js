...
(function () {
  ...
  async function checkAndShowFrozenBanner() {
    ...
    document.getElementById("bf-reactivate-btn")?.addEventListener("click", async () => {
      ...
      try {
        ...
        const { data: count } = await supa.rpc("reactivate_user_posts", { p_user_id: user.id });
  ...
  async function loadPosts(query = "") {
    ...
    const { data, error } = await supa
      .from("posts")
      .select("*")
      .or("sold.is.null,sold.eq.false")  // hide sold posts
      .eq("frozen", false)               // ← hide frozen posts
    ...
    // Show frozen banner if this user's account is frozen
