...
(function () {
  ...
  async function startConversationAndSendMessage(post) {
    ...
    try {
      ...
      if (msgErr) {
        ...
        alert("Message failed (likely RLS). Check Supabase policies.");
        return;
      }
      ...
      // Optional hooks if you later add an inbox/chat UI
  ...
  function openDetailPanel(post) {
    ...
    // Use the button, not random clicks on the whole panel.
    if (detailMessageBtn) {
      detailMessageBtn.disabled = (post.user_id === window.currentUser?.id);
      detailMessageBtn.onclick = (e) => {
        e.preventDefault();
        e.stopPropagation();
        startConversationAndSendMessage(post);
      };
    }

    // Don't let typing/clicking in the textarea trigger any parent handlers.
    if (chatInput) {
