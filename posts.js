...
(function () {
  ...
  function renderPostCard(p) {
    ...
    const isRequest = p.type === "requesting";
    const priceDisplay = p.price != null && p.price !== "" && p.price !== 0
      ? (isRequest ? `<span class="card-budget">Up to $${Number(p.price).toLocaleString()}</span>` : `<span class="card-budget">$${Number(p.price).toLocaleString()}</span>`)
      : (isRequest ? `<span class="card-budget" style="color:var(--muted)">Price open</span>` : "");
    ...
    return `
      <article class="post" data-post-id="${p.id}">
        ...
        <div class="post-body">
          <span class="post-type-pill ${isRequest ? "request" : "selling"}">${isRequest ? "🔍 Wanted" : "🏷️ For Sale"}</span>
          <h3>${p.title}</h3>
          <p>${p.description || ""}</p>
          <div class="post-meta-row">
            ${priceDisplay}
            <small style="color:var(--muted);font-size:11px;">${p.location_text || ""}</small>
          </div>
        </div>
      </article>
    `;
  ...
  async function openPostById(postId) {
    ...
    if (found) {
      ...
      // refreshPosts() lives in app.js, but we can load directly here.
    ...
    if (error || !data) {
      // fallback: set pending and force reload list
    ...
    if (window.setActiveView) window.setActiveView("posts");
    // set pending so loadPosts opens it after list is built
    window.__bf_pending_open_post_id = id;
    await loadPosts(window.__bf_last_search_query || "");
  }
