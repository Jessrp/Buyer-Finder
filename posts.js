...
(function () {
  ...
  function openModalForEdit(post) {
    ...
    if (post.image_urls) {
      if (Array.isArray(post.image_urls)) {
        editingPostImages = post.image_urls;
  ...
  async function savePost() {
    ...
    const payload = {
      ...
      description,
      price: price || null,
      type: window.activePostType.toLowerCase().startsWith("request")
        ? "requesting"
        : "selling",
      category: null,
      condition: null,
    ...
    try {
      if (editingPostId) {
        ...
        if (data && typeof window.tryCreateMatchesForNewPost === "function") {
          window.tryCreateMatchesForNewPost(data);
        }
      }
  ...
  async function loadPosts(query) {
    ...
    const filtered = (data || []).filter((p) => {
      const t = (p.type || "").toLowerCase() === "requesting" ? "request" : "selling";
  ...
  function renderPostCard(p) {
    ...
    if (p.image_urls && Array.isArray(p.image_urls) && p.image_urls.length) {
      img = `<img src="${p.image_urls[0]}" />`;
    }
