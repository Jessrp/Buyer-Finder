...
if (editingPost) {
  ...
  result = await supa.from("posts").insert(payload);
}

if (result.error) {
  postModalHint.textContent = result.error.message;
  return;
}
...
`;
  ...
  postsGrid.querySelectorAll(".post").forEach((card) => {
    ...
    if (editBtn && window.currentUser?.id === post.user_id) {
      editBtn.onclick = (e) => {
        e.stopPropagation();
        openModalForCreate(post);
      };
    }
  });
}
