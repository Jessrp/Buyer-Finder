...
(function () {
  ...
  async function markAsSold(post) {
    ...
    alert(`✅ "${post.title}" marked as sold${buyerName ? ` to ${buyerName}` : ""}!\n\nIt's been removed from the feed and saved to your transaction history in Settings.`);
    hideDetailPanel();
    loadPosts(window.__bf_last_search_query || "");
  }
  ...
  async function loadPosts(query = "") {
    ...
    const { data, error } = await supa
      .from("posts")
      .select("*")
      .eq("sold", false)   // ← hide sold posts from feed
  ...
  async function openPostById(postId) {
    ...
    if (error || !data) {
      window.__bf_pending_open_post_id = 
