...
(function () {
  ...
  async function loadMatches() {
    ...
    // Support both new schema (buy_post_id/sell_post_id, score) and legacy schema (post_a/post_b, "Score")
    const postIds = Array.from(new Set(
      matches
        .flatMap(m => [m.buy_post_id || m.post_a, m.sell_post_id || m.post_b])
        .filter(Boolean)
        .map(String)
    ));
    ...
    list.innerHTML = matches.map(m => {
      const buyId = m.buy_post_id || m.post_a;
      const sellId = m.sell_post_id || m.post_b;

      const buy = postsById[String(buyId)] || {};
      const sell = postsById[String(sellId)] || {};

      const s = (m.score ?? m.Score);
      const score = s != null ? `<span class="pill">Score ${s}</span>` : "";
