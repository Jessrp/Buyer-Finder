...
(function () {
  ...
  // If we're coming from Alerts, force-include the focused match even if it's below the threshold.
  // This guarantees "tap alert -> see that exact match in Matches".
  if (focusStr && !matches.some(m => String(m.id) === focusStr)) {
    try {
      const { data: focused, error: fErr } = await client
        .from("matches")
        .select("*")
        .eq("id", focusStr)
        .maybeSingle();
      ...
      if (!fErr && focused) {
        const isMine = focused.buyer_id === user.id || focused.seller_id === user.id;
        if (isMine) matches.unshift(focused);
      }
    } catch (e) {
      // ignore
    }
