...
// This file is NOT a module. It attaches to window.Matches.
(function () {
  ...
  async function loadMatches() {
    ...
    if (!supa) {
      list.innerHTML = "<p class='hint'>Supabase not ready yet.</p>";
      return;
    }
    ...
    const postIds = Array.from(
      new Set(
        matches
          .flatMap((m) => [m.buy_post_id, m.sell_post_id])
          .filter(Boolean)
          .map(String)
      )
    );
    ...
    if (postIds.length) {
      ...
      if (!pErr && posts) {
        postsById = Object.fromEntries(posts.map((p) => [String(p.id), p]));
      }
    }
    ...
    list.querySelectorAll(".match-item").forEach((el) => {
      el.onclick = async () => {
        ...
        if (window.Conversations?.getOrCreateConversation && window.Messages?.loadInbox) {
          try {
            const postId = m.buy_post_id || m.sell_post_id;
            const buyerId = m.buyer_id;
            const sellerId = m.seller_id;
            const convo = await window.Conversations.getOrCreateConversation({ postId, buyerId, sellerId });
            window.Messages.loadInbox();
            window.openConversation?.(convo.id);
            return;
          } catch (e) {
            console.error("Open convo from match failed:", e);
          }
        }
