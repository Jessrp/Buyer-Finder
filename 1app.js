...
async function initApp() {
    ...
    if (!supabase || typeof supabase.from !== 'function') {
        ...
        console.log('App will run in limited mode.');
        showAuthScreen();
        return;
    }
...
function setupEventListeners() {
    ...
    console.log('- Google:', signInGoogleAuth ? 'YES' : 'NO');
    console.log('- Email:', signInEmailAuth ? 'YES' : 'NO');
    console.log('- Guest:', continueGuestBtn ? 'YES' : 'NO');
    
    if (signInGoogleAuth) {
        signInGoogleAuth.addEventListener('click', function() {
    ...
    if (signInEmailAuth) {
        signInEmailAuth.addEventListener('click', function() {
...
function continueAsGuest() {
    ...
    loadPosts().catch(error => {
        console.error('Error loading posts as guest:', error);
    });
    console.log('=== continueAsGuest complete ===');
}
        ...
        if (container) {
            container.innerHTML = `
                <div class="empty-state">
                    <h2>👋 Welcome to BuyerFinder!</h2>
                    <p>Database connection not available.</p>
                    <p>Sign in to view and create posts.</p>
                </div>
            `;
        }
        return;
