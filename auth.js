...
function renderUserCard() {
  ...
  // Ensure avatar tap remains bound after re-render
  bindAvatarTap();
...
// --- BF_AVATAR_PICKER (tap avatar to change, mobile-safe) ---
function ensureAvatarPickerInput() {
  // Prefer existing settings input
  let input = document.getElementById("profile-avatar-input");

  // If input exists but is display:none, iOS may block programmatic click.
  // Keep it visually hidden but not display:none.
  if (input) {
  ...
  // Otherwise create a hidden input just for avatar picking
  input = document.createElement("input");
  input.type = "file";
  input.accept = "image/*";
  input.id = "bf-avatar-picker";
  input.style.position = "fixed";
  input.style.left = "-9999px";
  input.style.top = "0";
  input.style.width = "1px";
...
function bindAvatarTap() {
  ...
  document.addEventListener("click", async (e) => {
    const target = e.target && e.target.closest
      ? e.target.closest("#user-avatar, #user-avatar *, #settings-avatar, #settings-avatar *")
      : null;
    if (!target) return;
    ...
    const onChange = async () => {
      ...
      if (!isRealInput) {
        try { profileAvatarInput = input; } catch {}
      }
      await uploadAvatar();
    };
