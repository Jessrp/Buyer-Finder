...
let supa = initSupabaseClient();

// Retry a few times if the Supabase library loads late
if (!supa) {
  let tries = 0;
  const t = setInterval(() => {
    tries++;
    ...
    if (supa || tries >= 40) { // ~4s
      clearInterval(t);
      if (!supa) {
        console.error("❌ Supabase client never initialized (window.supabase missing).");
        alert("Supabase failed to load. Check network or CDN blocking.");
      }
    }
  }, 100);
}
