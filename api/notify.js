...
async function handleNewMessage(message) {
  ...
  const appUrl      = process.env.APP_URL || "https://buyerfinder.vercel.app";
...
async function sendEmail({ to, subject, body, appUrl }) {
  ...
  await resend.emails.send({
    from: `BuyerFinder <${process.env.NOTIFY_FROM_EMAIL}>`,
    to,
    subject,
    html: `
      <div style="font-family:sans-serif;max-width:480px;margin:0 auto;padding:24px;">
        <h2 style="color:#00dfa2;margin-bottom:8px;">BuyerFinder</h2>
        ...
        <p style="margin-top:24px;font-size:11px;color:#999;">
          You're receiving this because you have an account on BuyerFinder.<br/>
